...
public class AbTitleBar extends LinearLayout {
	...
	/** 左边图标的对齐参数. */
	private LinearLayout.LayoutParams logoLayoutParams = null;
	/** 右边布局的的对齐参数. */
	...
	public void ininTitleBar(Context context){
		...
		logoLayoutParams = new LinearLayout.LayoutParams(LayoutParams.WRAP_CONTENT, LayoutParams.WRAP_CONTENT);
		logoLayoutParams.gravity = Gravity.CENTER_VERTICAL;
		...
		this.addView(logoView,logoLayoutParams);
	 ...
	 */
	public void setTitleTextSize(int res) {
		this.titleTextBtn.setTextSize(TypedValue.COMPLEX_UNIT_PX,getResources().getDimensionPixelSize(res));
	}
	...
	public void setTitleTextColor(int res){
		this.titleTextBtn.setTextColor(res);
	}
	/**
	 * 描述：设置标题文字对齐方式
	 * 根据右边的具体情况判定方向：
	...
	public void setLogoMargin(int left,int top,int right,int bottom) {
		logoLayoutParams.setMargins(left, top, right, bottom);
	}
	/**
	 * 描述：获取标题Logo的View.
	 * @return the logo view
	...
	public void setLogo(int resId) {
		logoView.setVisibility(View.VISIBLE);
		logoView.setPadding((int)AbViewUtil.dip2px(mActivity, 16), 0, (int)AbViewUtil.dip2px(mActivity, 16), 0);
